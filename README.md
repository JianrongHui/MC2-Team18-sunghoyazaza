...
<p align="middle">"To test takers who are having a hard time getting sleeping habits!"</p>
<p align="middle">It's a service that uses ScreenTimeAPI to limit your smartphone to catch your sleeping habits!</p>
...
<p align="left">Our service is a smartphone usage restriction application using screen time to help test takers who frequently use their smartphones when they need to sleep and achieve their plans for the next day</p>
...
# 깃 협업 규칙 (Git Collaboration Regulations)

### 브랜치 규칙 (Branch Regulation)
...
- Feature 브랜치(feat)
    ...
    - 기능을 다 완성할 때까지 유지하고, 다 완성되면 dev 브랜치로 Pull Request(PR)를 생성한다.
    - 이슈번호 앞에 #을 붙이면 이슈 트랙킹 가능
- PR을 올릴 때는 자신이 한 일이 무엇인지 팀원이 쉽게 파악할 수 있도록 작성한다.(가능하면 스크린샷 까지… 최대한 자세하게 적어주세용..!)
- 정기회의를 통해 PR에 올라온 기능에 대한 피드백과 리뷰, 테스트를 진행한다. (2~3일에 한번, 혹은 요청에 따라서)
- 리뷰와 논의가 끝나면 dev 브랜치로 merge한다. (리뷰어가 진행)
- 리뷰 후 브랜치는 삭제한다.
- dev가 업데이트 되면 알림 후 각자의 브랜치에서 dev를 pull 받는다.
    - 이 과정에서 충돌이 생기면 코드 작성자와 함께 충돌 사항을 고친다.

### 커밋 규칙

- 커밋 메세지: `[#이슈번호] Type: Subject`
